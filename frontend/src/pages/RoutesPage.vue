...
import { Archive, Cable, CheckCircle2, Plus, Search, Pencil } from 'lucide-vue-next'
...
async function retire() { if (!selected.value) return; await ElMessageBox.confirm('退役后该线路不能再导入轨迹、设置基线或新建案例，且不可恢复，确认退役？', '退役线路', { type: 'warning', confirmButtonText: '确认退役', cancelButtonText: '取消' }); busy.value = true; try { selected.value = await store.retire(selected.value.id); ElMessage.success('线路已退役封存') } finally { busy.value = false } }
onMounted(search)
</script>
...
<template>
  ...
  <el-drawer v-model="detailOpen" :title="selected ? `${selected.route_code} / ${selected.name}` : '线路详情'" size="min(860px, 94vw)">
    <template #header><div class="drawer-title"><span>{{ selected ? `${selected.route_code} / ${selected.name}` : '线路详情' }}</span><span class="drawer-actions"><el-button v-if="selected && auth.canReview() && selected.route_status !== 'retired'" size="small" type="warning" plain :loading="busy" @click="retire"><Archive :size="14" />退役线路</el-button><el-button v-if="selected && auth.canAnalyze()" size="small" @click="openEdit"><Pencil :size="14" />编辑档案</el-button></span></div></template>
    ...
    <div class="history-list"><button v-for="trace in history" :key="trace.id" :class="{ active: preview?.trace.id === trace.id }" @click="showTrace(trace.id)"><span><strong>#{{ trace.id }} · {{ trace.wavelength_nm }} nm</strong><small>{{ new Date(trace.captured_at).toLocaleString() }}</small></span><span class="history-actions"><i v-if="selected?.baseline_trace_id === trace.id">当前基线</i><el-button v-else-if="auth.canReview() && selected?.route_status !== 'retired'" size="small" @click.stop="setBaseline(trace.id)">设为基线</el-button></span></button><div v-if="!history.length" class="empty-state"><span>该线路还没有轨迹记录。</span></div></div>
  </el-drawer>

  <el-dialog v-model="editOpen" title="编辑线路档案" width="min(620px, calc(100vw - 28px))">
    <el-form label-position="top"><div class="form-grid"><el-form-item label="线路名称"><el-input v-model="editForm.name" /></el-form-item><el-form-item label="线路长度（m）"><el-input-number v-model="editForm.length_m" :min="1" :max="500000" style="width:100%" /></el-form-item><el-form-item label="折射率"><el-input-number v-model="editForm.refractive_index" :min="1.3" :max="1.7" :precision="4" :step="0.001" style="width:100%" /></el-form-item><el-form-item label="发射端连接器"><el-input v-model="editForm.launch_connector" /></el-form-item><el-form-item label="线路状态"><el-select v-model="editForm.route_status" :disabled="editForm.route_status === 'retired'" style="width:100%"><el-option label="在线" value="active" /><el-option label="维护" value="maintenance" /><el-option v-if="editForm.route_status === 'retired'" label="退役（不可恢复）" value="retired" /></el-select><small v-if="editForm.route_status === 'retired'" class="muted">退役为终态，由「退役线路」操作写入，不可在编辑中变更。</small></el-form-item></div></el-form>
