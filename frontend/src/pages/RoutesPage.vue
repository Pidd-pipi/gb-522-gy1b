...
import { ArchiveRestore, Cable, CheckCircle2, LockKeyhole, Plus, Search, Pencil } from 'lucide-vue-next'
...
const routeStatusLabel: Record<FiberRoute['route_status'], string> = { active: '在线', maintenance: '维护', retired: '退役' }
...
async function retire() { if (!selected.value) return; await ElMessageBox.confirm('退役后线路统一封存：不能再导入轨迹、设置基线或新建定位案例；已有待复核案例仍可确认或关闭。若存在分析中的案例，本次退役会被拒绝。', '封存退役线路', { type: 'warning', confirmButtonText: '确认退役', cancelButtonText: '取消' }); const updated = await store.retire(selected.value.id); selected.value = updated; ElMessage.success('线路已退役封存') }
...
<template>
  ...
  <section class="content-band">
    ...
    <div class="data-surface">
      <el-table v-loading="store.loading" :data="store.items" row-key="id" @row-dblclick="inspect">
        ...
        <el-table-column label="状态" width="105"><template #default="scope"><span class="status-pill" :class="scope.row.route_status">{{ routeStatusLabel[scope.row.route_status as FiberRoute['route_status']] }}</span></template></el-table-column>
  ...
  <el-drawer v-model="detailOpen" :title="selected ? `${selected.route_code} / ${selected.name}` : '线路详情'" size="min(860px, 94vw)">
    <template #header><div class="drawer-title"><span>{{ selected ? `${selected.route_code} / ${selected.name}` : '线路详情' }}</span><span class="drawer-actions"><el-button v-if="selected && selected.route_status !== 'retired' && auth.canReview()" size="small" type="warning" plain :loading="busy" @click="retire"><ArchiveRestore :size="14" />退役封存</el-button><el-button v-if="selected && selected.route_status !== 'retired' && auth.canAnalyze()" size="small" @click="openEdit"><Pencil :size="14" />编辑档案</el-button></span></div></template>
    <div v-if="selected && selected.route_status === 'retired'" class="sealed-banner"><LockKeyhole :size="15" /><span>该线路已退役封存：不能再导入轨迹、设置基线或新建定位案例；已有待复核案例仍可确认或关闭，但不得重新分析。</span></div>
    <div v-if="selected" class="route-facts"><span><small>长度</small><strong>{{ selected.length_m.toLocaleString() }} m</strong></span><span><small>折射率</small><strong>{{ selected.refractive_index }}</strong></span><span><small>基线轨迹</small><strong>{{ selected.baseline_trace_id ? `#${selected.baseline_trace_id}` : '未设置' }}</strong></span><span><small>状态</small><strong>{{ routeStatusLabel[selected.route_status] }}</strong></span></div>
...
<style scoped>
.baseline { display:inline-flex; align-items:center; gap:5px; color:var(--accent); font-weight:700; }.muted { color:var(--text-muted); }.form-grid { display:grid; grid-template-columns:repeat(2,minmax(0,1fr)); gap:0 16px; }.sealed-banner { display:flex; align-items:center; gap:10px; margin-bottom:14px; padding:10px 14px; border:1px solid #d8b26b; background:#fbf3e2; color:#755310; font-size:13px; line-height:1.5; }.drawer-actions { display:flex; align-items:center; gap:8px; }.status-pill.active { color:#17604e; border-color:#9dcebd; background:#e8f4f0; }.status-pill.maintenance { color:#755310; border-color:#e0c16b; background:#fff5db; }.status-pill.retired { color:#8a3b3b; border-color:#d6a0a0; background:#f7e9e9; }.route-facts { display:grid; grid-template-columns:repeat(4,minmax(0,1fr)); margin-bottom:16px; border:1px solid var(--line); }.route-facts span { padding:12px 14px; border-right:1px solid var(--line); }.route-facts span:last-child { border-right:0; }.route-facts small,.route-facts strong { display:block; }.route-facts small { color:var(--text-muted); font-size:11px; }.route-facts strong { margin-top:4px; font-size:15px; }.history-title { margin:24px 0 10px; font-size:15px; }.history-list { border-top:1px solid var(--line); }.history-list>button { width:100%; min-height:58px; display:flex; align-items:center; justify-content:space-between; gap:12px; padding:9px 12px; border:0; border-bottom:1px solid var(--line); background:transparent; color:var(--text); text-align:left; cursor:pointer; }.history-list>button:hover,.history-list>button.active { background:var(--surface-strong); }.history-list strong,.history-list small { display:block; }.history-list small { margin-top:3px; color:var(--text-muted); }.history-actions { display:flex; align-items:center; gap:8px; }.history-actions i { color:var(--accent); font-size:11px; font-style:normal; font-weight:800; }
