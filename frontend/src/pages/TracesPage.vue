...
const importableRoutes = computed(() => routes.items.filter((item) => item.route_status !== 'retired'))
...
onMounted(async () => { await routes.fetch({ page_size:100 }); if (importableRoutes.value[0]) form.route_id = importableRoutes.value[0].id; await filter() })
</script>
