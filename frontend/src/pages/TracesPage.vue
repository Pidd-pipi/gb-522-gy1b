...
const firstOpenRouteId = () => routes.items.find((item) => item.route_status !== 'retired')?.id ?? routes.items[0]?.id ?? 0
...
function openImport() { if (!form.route_id || routes.items.find((item) => item.id === form.route_id)?.route_status === 'retired') form.route_id = firstOpenRouteId(); importOpen.value = true; samplePoints() }
...
onMounted(async () => { await routes.fetch({ page_size:100 }); form.route_id = firstOpenRouteId(); await filter() })
</script>
...
<template>
  ...
  <section class="content-band trace-workspace">
    <aside class="trace-index data-surface">
      <div class="index-head"><strong>轨迹列表</strong><el-select v-model="routeFilter" clearable placeholder="全部线路" size="small" @change="filter"><el-option v-for="route in routes.items" :key="route.id" :label="route.route_status === 'retired' ? `${route.route_code}（退役）` : route.route_code" :value="route.id" /></el-select></div>
