...
const routeById = (id: number) => routes.items.find((item) => item.id === id)
...
function openCreate() { if (!form.route_id || routeById(form.route_id)?.route_status === 'retired') form.route_id = routes.items.find((item) => item.route_status !== 'retired')?.id ?? 0; chooseRoute(); createOpen.value = true }
...
onMounted(async()=>{ await Promise.all([routes.fetch({page_size:100}),traces.fetch({page_size:100}),search()]); const openRoute = routes.items.find((item)=>item.route_status!=='retired') ?? routes.items[0]; if(openRoute){form.route_id=openRoute.id;chooseRoute()} })
</script>

<template>
  <PageHeader title="定位案例" eyebrow="LOCALIZATION CASES" description="基线差异仅作分析参考，由复核人员形成最终结论。"><el-button v-if="auth.canAnalyze()" type="primary" @click="openCreate"><Plus :size="16" />新建案例</el-button></PageHeader>
  <section class="content-band">
    ...
    <div class="data-surface">
      <el-table v-loading="cases.loading" :data="cases.items" row-key="id">
        ...
        <el-table-column label="线路" width="125"><template #default="scope"><span :class="{ 'retired-route': routeById(scope.row.route_id)?.route_status === 'retired' }">{{ routes.items.find(r=>r.id===scope.row.route_id)?.route_code ?? `#${scope.row.route_id}` }}</span></template></el-table-column>
        ...
        <el-table-column label="操作" width="190" fixed="right"><template #default="scope"><el-button v-if="scope.row.case_status==='draft' && routeById(scope.row.route_id)?.route_status !== 'retired' && auth.canAnalyze()" link type="primary" :loading="busy" @click="analyze(scope.row)"><Play :size="14" />分析</el-button><span v-else-if="scope.row.case_status==='draft' && routeById(scope.row.route_id)?.route_status === 'retired'" class="muted">线路已封存</span><el-button v-if="scope.row.case_status==='pending_review' && auth.canReview()" link type="primary" @click="confirm(scope.row)"><ClipboardCheck :size="14" />确认</el-button><el-button v-if="scope.row.case_status==='confirmed' && auth.canReview()" link type="danger" @click="close(scope.row)"><LockKeyhole :size="14" />关闭</el-button><span v-if="scope.row.case_status==='closed'" class="muted">已归档</span></template></el-table-column>
