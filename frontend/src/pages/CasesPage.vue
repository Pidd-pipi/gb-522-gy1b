...
const creatableRoutes = computed(() => routes.items.filter((item) => item.route_status !== 'retired'))
const isRetiredRoute = (routeId: number) => routes.items.find((item) => item.id === routeId)?.route_status === 'retired'
...
onMounted(async()=>{ await Promise.all([routes.fetch({page_size:100}),traces.fetch({page_size:100}),search()]); if(creatableRoutes.value[0]){form.route_id=creatableRoutes.value[0].id;chooseRoute()} })
</script>
